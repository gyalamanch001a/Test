...
def analyze_url(url, current_text, recommended_text):
    ...
    current_text.insert(tk.END, "Current Sentences:\n")
    recommended_text.insert(tk.END, "Recommended Sentences:\n")

    for sentence in sentence_recommendations:
        wrapped_sentence = textwrap.fill(sentence, width=70)
        current_text.insert(tk.END, f"- {wrapped_sentence}\n")
        recommended_text.insert(tk.END, f"- {wrapped_sentence}\n")
...
def main():
    ...
    current_text = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=40, height=20)
    current_text.pack(side=tk.LEFT, padx=10, fill=tk.BOTH, expand=True)

    recommended_text = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=40, height=20)
